> 最近在总结图计算Pregel框架相关知识点，本来打算自己动手写一篇的，但发现知乎中发现已有大神总结的非常好了，这里我就顺手牵羊，直接转载过来了，就不算另写了。
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-vertex-status-transition.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">Vertex 的状态变化</div>
</center>

前面提到 Vertex 会有状态变化，这个概念也十分简单:

1. 当一个 Vertex 在上一步当中没有接收到消息，或者算法自己决定不再向外发送消息，它可以被转变为 Inactive 的。 在 Pregel 的术语当中，这被称为 Vote to halt
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-without-combiners.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">通过网络传递 Message</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-with-combiners.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">使用 Combiner 实现预先聚合</div>
</center>
...
<center>
    ...
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-aggregator.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">使用 Aggregator 跟踪全局信息</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:90%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-topology-mutation-requests.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">通过发送特殊的消息实现图拓扑的修改</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-graph-partitions.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-master-worker-model.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
...
3. 如果 Attach 在某一个 Vertex 上的 Message 在上一步当中变大(变小)了， 它就会把新的值发送给所有相邻的节点，否则它会执行 Vote to halt 来 Inactivate 自己
...
值得注意的是，Pregel 实现的连通分量算法在超大规模的图上仍然有可扩展性的瓶颈， Google 之后发表了论文 [Connected Components in MapReduce and Beyond](https://research.google.com/pubs/pub43122.html) 加以解决。

### 计算单源最短路

最短路的经典单机算法包括 [Dijkstra](https://en.wikipedia.org/wiki/Dijkstra's_algorithm)、 [Bellman-Ford](https://en.wikipedia.org/wiki/Bellman–Ford_algorithm) 等。 在图规模巨大无法被放入内存的场景下，看到 Pregel 的消息传递模型仍然适用。具体的方法是:
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-shortest-path-step-1.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">Pregel 计算最短路: 步骤1</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-shortest-path-step-2.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">Pregel 计算最短路: 步骤2</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-shortest-path-step-3.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">Pregel 计算最短路: 步骤3</div>
</center>
...
[PageRank](https://en.wikipedia.org/wiki/Pagerank) 可能是 Pregel 最典型的应用案例。 因为它本身的原理就是通过不断将自身的权值以消息的形式发送出去而完成计算的。
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-pagerank.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">PageRank：前半部分</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-pagerank-terminate.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">PageRank：后半部分</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-user-tracking-problem.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">用户追踪画像问题</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-user-tracking-sessionize.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">用户离散访问信息的初步处理</div>
</center>
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-user-tracking-feature-extraction.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">通过计算无向图连通分量聚合用户信息</div>
</center>
...
在这里有一点扩展优化。设想我们有 O(N)O(N) 个 Session，要找出它们两两之间的关系， 需要耗费的是 O(N2)O(N2) 的时间，这对于仅有 106106 个 Session 的规模都是相当不可行的。 我所想到的一种优化是：

1. 只选择 nn 个重要的特征作为参考
2. 定义两个 Session 是匹配的，当有大于等于 xx 个特征相匹配
3. 对于收集到的一个 Session 的特征，枚举它恰好 xx 个特征的子集合
4. 匹配这些包含恰好 xx 个特征的子集合，而不是 Session 本身
...
<center>
    <img style="border-radius: 0.3125em;zoom:80%;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src=".images/pregel-user-feature-match.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">枚举固定数量特征子集合进行匹配</div>
</center>

假设 n=5n=5 且 x=2x=2，也就是说对于任意两个 Session，我们只考虑它们所拥有的 5 个固定的特征， 当等于或大于 2 个特征匹配，我们就认为两个 Session 匹配。如果我们有 O(N)O(N) 个 Session， 因 𝙲25=10C52=10 那么我们将会得到 O(10×N)O(10×N) 个这样的子集合。
